<template>
  <svg
    width="24"
    height="24"
    viewBox="0 0 24 24"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      fill-rule="evenodd"
      clip-rule="evenodd"
      d="M5.84025 8H18.1597C19.1999 8 20.0664 8.79732 20.1528 9.83391L20.8195 17.8339C20.9112 18.9347 20.0932 19.9014 18.9925 19.9931C18.9372 19.9977 18.8818 20 18.8264 20H5.17358C4.06901 20 3.17358 19.1046 3.17358 18C3.17358 17.9446 3.17589 17.8892 3.18049 17.8339L3.84716 9.83391C3.93354 8.79732 4.80007 8 5.84025 8ZM10.5 10C10.2238 10 9.99998 10.2239 9.99998 10.5V11.5C9.99998 11.7761 10.2238 12 10.5 12H13.5C13.7761 12 14 11.7761 14 11.5V10.5C14 10.2239 13.7761 10 13.5 10H10.5Z"
      fill="black"
    />
    <path
      opacity="0.3"
      d="M10 8H8V7C8 5.34315 9.34315 4 11 4H13C14.6569 4 16 5.34315 16 7V8H14V7C14 6.44772 13.5523 6 13 6H11C10.4477 6 10 6.44772 10 7V8Z"
      fill="black"
    />
  </svg>
</template>

<style lang="scss" scoped>
@import "@/assets/scss/components/navbar/menu-item.scss";
</style>
